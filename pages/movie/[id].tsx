...
function Movie({ movie }: Props) {
  ...
  const index = movie.videos.results.findIndex((element:any) => element.type === "Trailer")
  return (
        ...
        <div className="absolute inset-y-28 md:inset-y-auto md:bottom-10 inset-x-4 md:inset-x-12 space-y-6 z-50">
          ...
          <div className="flex items-center space-x-3 md:space-x-5">
            ...
            <button
              ...
              onClick={() => (setShowPlayer(true), console.log('pressed'))}
            >
...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  const path1 = popularMovies.results.map((movie: any) => ({
    params: { id: movie.id.toString() },
  }));
  const path2 = top_ratedMovies.results.map((movie: any) => ({
    params: { id: movie.id.toString() },
  }));
...
export const getStaticProps: GetStaticProps = async ({
  ...
  const res = await fetch(`
    https://api.themoviedb.org/3/movie/${params.id}?api_key=${process.env.API_KEY}&language=en-US&append_to_response=videos`);
  const movie = await res.json();
//   console.log("movie", movie);

  // Pass post data to the page via props
  return { props: { movie }, revalidate: 100,};
};
